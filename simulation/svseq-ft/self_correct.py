...
def get_csv(com_clus_set,sam_file):


    csv_set = []
    print("com_clus_len:"+str(len(com_clus_set)))
    cir_num = 0

    for clus_elem in com_clus_set:
        fout_csv = open(sam_file+"_csv", "a")
        cir_num += 1
        wait_correct = []
        ...
        if(cir_num%10 == 0):
            print(cir_num)
        fout_csv.write(str(clus_elem.left_bp)+" "+str(clus_elem.right_bp)+" "+correct_str+"\n")
        fout_csv.close()
    return csv_set

...
def get_com_indel(feature_list_set,total_indel_set,readInfo):

    clf = joblib.load("svm.model")
    com_indel_set = []
    for (feature,indel) in zip(feature_list_set,total_indel_set):
        if(clf.predict(np.array(feature).reshape(1,-1))):           #如果是复杂indel
